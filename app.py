...
# ——— DATABASE (fresh cursor every run) ———
@st.cache_resource
def get_db():
...
conn, c = get_db()   # ← this fixes the "str object has no attribute 'executemany'" forever
...
if uploaded:
    try:
        ...
        for col in tmp.columns:
            ...
            if "weight" in l: rename_map[col] = "weight"
        ...
        conn.commit()
        st.success(f"Imported {len(tmp)} rows successfully!")
        st.rerun()
...
with c1:
    ...
    st.metric("Latest Weight", m["latest"] + " lbs" if m["latest"] != "—" else "—")
...
with c2:
    ...
    st.metric("Latest Weight", j["latest"] + " lbs" if j["latest"] != "—" else "—")
...
st.dataframe(df.sort_values("date", ascending=False).head(10)[["user","date","weight"]], hide_index=True, use_container_width=True)
